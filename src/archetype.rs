...
impl Archetype {
    ...
    /// Clones the archetype or returns the first
        ...
        let new_data: Box<[Data]> = self
            ...
            .map(|(info, old)| {
                let storage: NonNull<u8> = if info.layout.size() == 0 {
                    // This is a zero-sized type, so we don't need to allocate or copy any data
                    NonNull::new(info.layout.align() as *mut u8).unwrap()
                } else {
                    // Allocate memory for the cloned data
                    let layout = Layout::from_size_align(
                        info.layout.size() * num_entities,
                        info.layout.align(),
                    )
                    .unwrap();
                    let new_storage = {
                    ...
                    // Retrieve the dynamic clone function for the component type and use it
                    // to call the
                    ...
                    unsafe {
                        clone_fn.call(old.storage.as_ptr(), new_storage.as_ptr(), num_entities)
                    };
